        ...
        foreach my $test_case ( [ "0", 0 ], [ "5", 5 ] ) {
          ...
          SKIP: {
                ...
                is( $status, $expected_status,
                    "$label: correct status returned" );

                $env->import_envs;
                my $argout = $ENV{"GETENV_ARG"};
                is( $ENV{"GETENV_TEST"}, 'bogus',
                    "$label: GETENV_TEST survived system call" );
                is( $ENV{"GETENV_ARG"}, $arg,
                    "$label: GETENV_ARG set correctly in system call" );
            }
        }
    }
}
