  ...
  SKIP:
    {
        ...
        for my $startup ( 0, 1 ) {
          ...
          SKIP: {
                ...
                my $envs = $env->envs;
                ok(
                    !exists $envs->{SHELL_GETENV_TEST},
                    "$shell: startup=$startup; unset"
                );
                ok(
                    exists $envs->{SHELL_GETENV}
                ...
                if ( $info->{Funky} ) {
                    ok(
                        exists $envs->{$FunkyEnv}
                          && $envs->{$FunkyEnv} eq $FunkyEnv,
      ...
      SKIP:
        {
            ...
            ok(
                exists $envs->{SHELL_GETENV} && $envs->{SHELL_GETENV} eq $shell,
                "$shell: expect;  set"
            );
